...
# Import all 11 of our exercise trackers
...
from app.engine.excercises.plank import PlankTracker
from app.engine.excercises.lunge import LungeTracker
from app.engine.excercises.high_knees import HighKneesTracker
from app.engine.excercises.deadlift import DeadliftTracker
...
def run_local_test():
    ...
    # Dictionary holding all 11 trackers!
    exercises = {
        ...
        '8': {"name": "Plank (30s)", "tracker": PlankTracker(target_time=30)},
        '9': {"name": "Lunges", "tracker": LungeTracker(target_reps=10)},
        '0': {"name": "High Knees", "tracker": HighKneesTracker(target_reps=20)},
        '-': {"name": "Deadlift", "tracker": DeadliftTracker(target_reps=8)}
    }
    ...
    print("Press 1-9, 0, or - to switch exercises. Press 'q' to quit.")
    ...
    while cap.isOpened():
        ...
        if landmarks:
            ...
            # Left side UI
            cv2.putText(annotated_frame, f"L-REPS/TIME: {left_data['reps']}", (10, 80), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(annotated_frame, f"L-STAGE: {left_data['stage']}", (10, 110), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 165, 0), 2)
            
            # Right side UI
            cv2.putText(annotated_frame, f"R-REPS/TIME: {right_data['reps']}", (400, 80), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(annotated_frame, f"R-STAGE: {right_data['stage']}", (400, 110), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 165, 0), 2)

            # Form Feedback (Anti-Cheat)
        ...
        # Menu split into two lines to accommodate all 11 modules
        menu_text_1 = "[1]Bicep [2]Squat [3]Jacks [4]Push-Up [5]Press [6]Sit-Up"
        menu_text_2 = "[7]LatRaise [8]Plank [9]Lunge [0]Knees [-]Deadlift [Q]Quit"
        ...
        elif key == ord('8'): current_mode = '8'
        elif key == ord('9'): current_mode = '9'
        elif key == ord('0'): current_mode = '0'
        elif key == ord('-'): current_mode = '-'
